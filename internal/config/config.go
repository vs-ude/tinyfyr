...
func getFyrDirectory(path string, err error) string {
	...
	return filepath.Join(path, "tinyfyr")
}
...
// FyrBase returns the path specified in the TFBASE environment variable.
// It should usually point to the installation path of the Fyr library and runtime.
func FyrBase() string {
	return config.FyrBase
}

// FyrPath returns the path specified in the TFPATH environment variable.
